...
(function($) {
	...
	jQuery(window).load(function(){
		...
		jQuery('.theme-feature .buttontoggler').click(function(){
			if(jQuery('.theme-feature').hasClass('open')) {
				jQuery('.theme-feature').removeClass('open');
			} else {
				jQuery('.theme-feature').addClass('open');
			}
		});
		...
		jQuery('#bgsolid a').each(function(){
			jQuery(this).click(function(event){
				...
				setCookie('header_layout', 'second', 1);
			});
		});
		//Change class image
