...
class VI_SCT_SALES_COUNTDOWN_TIMER_Frontend_Product_Countdown {
	...
	public function shortcode_init() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'sctv_reset_schedule' ), 99 );
	}
	...
	public function register_shortcode( $atts ) {
		...
		if ( $countdown_enable ) {
			...
			$sale_from_time = date( 'H:i', $sale_from );
			$sale_to_time   = date( 'H:i', $sale_to );
...
//			$sale_to_time   = woo_ctr_time_revert( $sale_to_time );
			$shortcode_atts = array(
				...
				'sale_from_time'             => $sale_from_time,
				'sale_to_time'               => $sale_to_time,
