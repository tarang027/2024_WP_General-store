...
//            /*countdown timer position*/
		...
		if ( is_product() && is_single() ) {
			...
			if ( $product ) {
				if ( ! $product->is_type( 'variable' ) ) {
					if ( ! $product->is_in_stock() || ! $product->get_sale_price( 'edit' ) ) {
						return;
					}
					...
					$default_attribute = method_exists( $product,
						'get_default_attributes' ) ? $product->get_default_attributes() : $product->get_variation_default_attributes();
					if ( ! $variation_id || count( $default_attribute ) ) {
						return;
					}
					$variation = wc_get_product( $variation_id );
					if ( ! $variation || ! $variation->is_in_stock() || ! $variation->get_sale_price( 'edit' ) ) {
						return;
					}
	...
	public function woocommerce_single_product_summary() {
		...
		if ( $this->id && $this->position == 'product_image' ) {
			...
			$shortcode = do_shortcode( '[sctv_product_countdown_timer is_single ="1" product_id="' . $this->product_id . '" progress_bar_enable="1" progress_bar_position ="' . $this->pg_position . '" countdown_enable="1"  countdown_id = "' . $this->id . '"]' );
//			$progress_bar = do_shortcode( '[sctv_product_countdown_timer is_single ="1" product_id="' . $this->product_id . '" progress_bar_enable="1" progress_bar_position ="' . $this->pg_position . '" countdown_enable=""  countdown_id = "' . $this->id . '"]' );
//			if ( $this->pg_position === 'above_countdown' ) {
//				$html = str_replace( '</figure>',
//					'</figure>' . '<div class="woo-sctr-single-product-container"><div class="woo-sctr-countdown-timer-product-image-wrap">'. $progress_bar. $countdown . '</div></div>',
//					$html );
//			} else {
//				$html = str_replace( '</figure>',
//					'</figure><div class="woo-sctr-single-product-container"><div class="woo-sctr-countdown-timer-product-image-wrap">' . $countdown .$progress_bar. '</div></div>',
//					$html );
//			}
			$html = str_replace( '</figure>',
