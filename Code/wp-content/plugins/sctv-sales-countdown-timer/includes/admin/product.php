...
class VI_SCT_SALES_COUNTDOWN_TIMER_Admin_Product {
	...
	public function woocommerce_product_save_countdown(
		$product_id,
		$sale_price,
		$regular_price,
		$gmt_offset,
		...
		if (count($schedule)){
			foreach ($schedule as $key => $schedule_data){
				...
				if ($schedule_hook==='sctv_schedule_sale_product' ){
					...
					if ($schedule_time == $date_on_sale_from1){
						$schedule_sale = array(
							'key'=>$key,
							'arg'=>$schedule[ $key ]['arg'],
						);
					}
				}
				if ( $schedule_hook==='sctv_schedule_remove_sale_product'){
					...
					if ($schedule_time == $date_on_sale_to1 ){
		...
		if ( $change ) {
			if ( $date_on_sale_from && $date_on_sale_to ) {
				...
				if ( $now < $date_on_sale_from ) {
					update_post_meta( $product_id, '_price', $regular_price );
					if (!empty($schedule_sale)){
						wp_unschedule_event($date_on_sale_from1, 'sctv_schedule_sale_product',  $schedule_sale['arg']  );
						$schedule_sale_arg = $schedule_sale['arg'][0];
						$schedule_sale_arg[]=$product_id;
						$schedule[$schedule_sale['key']]['arg']= array(array_unique($schedule_sale_arg));
						wp_schedule_single_event( $date_on_sale_from1, 'sctv_schedule_sale_product', array(array_unique($schedule_sale_arg)));
					}else {
						...
						$schedule[]= array(
							...
							'arg'=> array(array($product_id)),
						);
					}
				}
				if (!empty($schedule_remove_sale)){
					wp_unschedule_event($date_on_sale_to1, 'sctv_schedule_remove_sale_product', $schedule_remove_sale['arg']  );
					$schedule_remove_sale_arg =$schedule_remove_sale['arg'][0];
					$schedule_remove_sale_arg[]=$product_id;
					$schedule[$schedule_remove_sale['key']]['arg']=array(array_unique($schedule_remove_sale_arg));
					wp_schedule_single_event( $date_on_sale_to1, 'sctv_schedule_remove_sale_product', array(array_unique($schedule_remove_sale_arg)) );
				}else {
					$schedule[]= array(
			...
			update_post_meta( $product_id, '_sale_price_times_from', isset( $_REQUEST['_sale_price_times_from'] ) ? sanitize_text_field( $_REQUEST['_sale_price_times_from'] ) : '00:00' );
			update_post_meta( $product_id, '_sale_price_times_to', isset( $_REQUEST['_sale_price_times_to'] ) ? sanitize_text_field( $_REQUEST['_sale_price_times_to'] ) : '00:00' );
			...
			if ( get_post_meta( $product_id, '_sale_price_dates_from' ) ) {
	...
	public function woocommerce_product_bulk_edit_save( $product ) {
		if ( isset( $_REQUEST['_woo_ctr_select_change_setting'] ) && $_REQUEST['_woo_ctr_select_change_setting'] ) {
			$gmt_offset            = get_option( 'gmt_offset' );
			$date_on_sale_from     = isset( $_REQUEST['_sale_price_dates_from'] ) ? strtotime( $_REQUEST['_sale_price_dates_from'] ) : '';
			$date_on_sale_to       = isset( $_REQUEST['_sale_price_dates_to'] ) ? strtotime( $_REQUEST['_sale_price_dates_to'] ) : '';
			$sale_price_times_from = isset( $_REQUEST['_sale_price_times_from'] ) ? woo_ctr_time( sanitize_text_field( $_REQUEST['_sale_price_times_from'] ) ) : '';
			$sale_price_times_to   = isset( $_REQUEST['_sale_price_times_to'] ) ? woo_ctr_time( sanitize_text_field( $_REQUEST['_sale_price_times_to'] ) ) : '';
			if ( $product->is_type( 'simple' ) || $product->is_type( 'external' ) ) {
				...
				if ( $sale_price && $regular_price ) {
					$this->woocommerce_product_save_countdown( $product_id, $sale_price, $regular_price, $gmt_offset, $date_on_sale_from, $date_on_sale_to, $sale_price_times_from, $sale_price_times_to, true );
				} else {
					$this->woocommerce_product_save_countdown( $product_id,
						$sale_price,
						$regular_price,
						$gmt_offset,
						$date_on_sale_from,
			...
			} elseif ( $product->is_type( 'variable' ) && sanitize_text_field( $_REQUEST['_woo_ctr_select_change_setting'] ) === '2' ) {
				$variation_ids = method_exists( $product,
					'get_children' ) ? $product->get_children() : $product->children;
				if ( $variation_ids && is_array( $variation_ids ) && count( $variation_ids ) ) {
					foreach ( $variation_ids as $variation_id ) {
						$sale_price_v    = get_post_meta( $variation_id, '_sale_price', true );
						$regular_price_v = get_post_meta( $variation_id, '_regular_price', true );
						if ( $sale_price_v && $regular_price_v ) {
							$this->woocommerce_product_save_countdown( $variation_id,
								$sale_price_v,
								$regular_price_v,
								$gmt_offset,
								$date_on_sale_from,
	...
	public function admin_enqueue_scripts() {
		...
		if ( $screen->id == 'product' ) {
			wp_enqueue_script( 'sales-countdown-timer-admin-product',
				VI_SCT_SALES_COUNTDOWN_TIMER_JS . 'sales-countdown-timer-admin-product.js',
				array( 'jquery' ) );
			wp_enqueue_style( 'sales-countdown-timer-admin-product',
				VI_SCT_SALES_COUNTDOWN_TIMER_CSS . 'sales-countdown-timer-admin-product.css' );
		}
	}
	...
	public function woocommerce_process_product_meta_simple( $post_id ) {
		$gmt_offset            = get_option( 'gmt_offset' );
		$date_on_sale_from     = isset( $_POST['_sale_price_dates_from'] ) ? strtotime( $_POST['_sale_price_dates_from'] ) : '';
		$date_on_sale_to       = isset( $_POST['_sale_price_dates_to'] ) ? strtotime( $_POST['_sale_price_dates_to'] ) : '';
		...
		if ( $sale_price && $regular_price ) {
			if ( $date_on_sale_from && $date_on_sale_to ) {
				...
				if ( $now < $date_on_sale_from ) {
					...
					if (!empty($schedule_sale)){
						$a1=wp_unschedule_event($date_on_sale_from1, 'sctv_schedule_sale_product',  $schedule_sale['arg']  );
						$schedule_sale_arg = $schedule_sale['arg'][0];
						$schedule_sale_arg[]=$post_id;
						$schedule[$schedule_sale['key']]['arg']= array(array_unique($schedule_sale_arg));
						wp_schedule_single_event( $date_on_sale_from1, 'sctv_schedule_sale_product', array(array_unique($schedule_sale_arg)));
					}else {
				...
				if (!empty($schedule_remove_sale)){
					wp_unschedule_event($date_on_sale_to1, 'sctv_schedule_remove_sale_product', $schedule_remove_sale['arg']  );
					$schedule_remove_sale_arg =$schedule_remove_sale['arg'][0];
					$schedule_remove_sale_arg[]=$post_id;
					$schedule[$schedule_remove_sale['key']]['arg']=array(array_unique($schedule_remove_sale_arg));
					wp_schedule_single_event( $date_on_sale_to1, 'sctv_schedule_remove_sale_product', array(array_unique($schedule_remove_sale_arg)) );
				}else {
					$schedule[]= array(
			...
			if ( get_post_meta( $post_id, '_sale_price_dates_from',true ) ) {
				delete_post_meta( $post_id, '_sale_price_dates_from' );
				delete_post_meta( $post_id, '_sale_price_dates_to' );
				delete_post_meta( $post_id, '_sale_price_times_from' );
				delete_post_meta( $post_id, '_sale_price_times_to' );
			}
		}
		update_option('sctv_schedule_sales',array_values($schedule));
	...
	public function woocommerce_save_product_variation( $variation_id, $i ) {
		$gmt_offset            = get_option( 'gmt_offset' );
		$date_on_sale_from     = isset( $_POST['variable_sale_price_dates_from'][ $i ] ) ? strtotime( $_POST['variable_sale_price_dates_from'][ $i ] ) : '';
		$date_on_sale_to       = isset( $_POST['variable_sale_price_dates_to'][ $i ] ) ? strtotime( $_POST['variable_sale_price_dates_to'][ $i ] ) : '';
		...
		if ( $sale_price && $regular_price ) {
			...
			if ( $date_on_sale_from && $date_on_sale_to ) {
				if ( $now < $date_on_sale_from ) {
					...
					if (!empty($schedule_sale)){
						wp_unschedule_event($date_on_sale_from1, 'sctv_schedule_sale_product',  $schedule_sale['arg']  );
						$schedule_sale_arg = $schedule_sale['arg'][0];
						$schedule_sale_arg[]=$variation_id;
						$schedule[$schedule_sale['key']]['arg']= array(array_unique($schedule_sale_arg));
						wp_schedule_single_event( $date_on_sale_from1, 'sctv_schedule_sale_product', array(array_unique($schedule_sale_arg)));
					}else {
				...
				if (!empty($schedule_remove_sale)){
					wp_unschedule_event($date_on_sale_to1, 'sctv_schedule_remove_sale_product', $schedule_remove_sale['arg']  );
					$schedule_remove_sale_arg =$schedule_remove_sale['arg'][0];
					$schedule_remove_sale_arg[]=$variation_id;
					$schedule[$schedule_remove_sale['key']]['arg']=array(array_unique($schedule_remove_sale_arg));
					wp_schedule_single_event( $date_on_sale_to1, 'sctv_schedule_remove_sale_product', array(array_unique($schedule_remove_sale_arg)) );
				}else {
					$schedule[]= array(
			...
			delete_post_meta( $variation_id, '_sale_price_times_from' );
			delete_post_meta( $variation_id, '_sale_price_times_to' );
		}
		update_option('sctv_schedule_sales',$schedule);
